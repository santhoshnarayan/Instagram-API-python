...
class InstagramAPI:
    ...
    #DEPRECATED. Allow users set device settings by INSTANCE instead of class-level
    DEVICE_SETTINTS = {
        'manufacturer': 'Xiaomi',
        'model': 'HM 1SW',
        'android_version': 18,
    ...
    USER_AGENT      = 'Instagram {apk_version} Android ({android_version}/{android_release}; {dpi}; {resolution}; {manufacturer}; {model}; {device}; {cpu}; {locale})'
    APK_VERSION     = '10.26.0'
    ...
    def __init__(self, username, password, debug=False, IGDataPath=None,
        ...
        #avoid generating random device_id and uuid if you already login with previous settings
        self.device_id = device_id or self.generateDeviceId(m.hexdigest())
        self.uuid      = uuid
        self.setUser(username, password,uuid=uuid)
        
        #let user define what kind of device is using
    ...
    def setUser(self, username, password, uuid=None):
        self.username = username
        self.password = password
        self.uuid     = uuid or self.generateUUID(True)
    ...
    def generateUUID(self, type):
        generated_uuid = self.uuid or str(uuid.uuid4())
        if (type):
    ...
    def SendRequest(self, endpoint, post=None, login=False):
        ...
        headers_update = {
           ...
           'Cookie2': '$Version=1',
           'Accept-Language': self.locale, #'en_US',
           'User-Agent': self.user_agent()
        }
        self.s.headers.update(headers_update)
        
        _url = self.API_URL + endpoint
        if self.debug:
            print("headers:{}".format(self.s.headers))
            print("url:{}".format(_url))
        while True:
            try:
                if (post is not None):
                    response = self.s.post(_url, data=post, verify=verify)
                else:
                    response = self.s.get(_url, verify=verify)
                break
